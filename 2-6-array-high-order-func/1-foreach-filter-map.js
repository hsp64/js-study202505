...
// 매핑 -> 배열길이만큼 x
// 필터링 -> 배열길이만큼 x
//필터링을 먼저하면 매핑은 필터된 길이만큼 이기때문에 반복횟수가 줄어든다 o
...
// petName : petName 처럼 프로퍼티 키가 같을 시 생략가능하다. petAge 또한 그러하다.
const myDog = {
    petName,
    petAge
};
...
const finalUserList2 = userList
    .filter(user => user.salary >= 3000000 && user.age >= 30 && user.age < 40)
    .map(({age, job, salary, userName:name}) => ({name, job, salary, age}));
...
const distinctCities = [...new Set(cities)]; // 중복제거 set 자바에서 다시 본다
// distinctCities.push('ddd'); set은 배열이아니라 푸쉬가 안된다
distinctCities.push('ddd'); // 위에 배열이라고 [...  ] 해야 푸쉬가 가능하다
